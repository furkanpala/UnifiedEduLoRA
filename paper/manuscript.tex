...
% ── Layout helpers ────────────────────────────────────────────
\usepackage{booktabs}
\usepackage{array}      % needed for >{\raggedright\arraybackslash} in p-column specs
\usepackage{multirow}
\usepackage{enumitem}
\usepackage{xcolor}
...
educational materials supports formative assessment
at scale, but the quality of a locally trained AQAG
system is bounded by the volume and diversity of
one institution's curriculum and by the compute
available to fine-tune a generative model. This
produces an \textbf{equity gap} between
well-resourced and resource-constrained
institutions. Large language models would close
that gap in principle, but their compute,
licensing, and data-governance requirements put
direct deployment out of reach for most
institutions; small language models (SLMs) run on
commodity hardware but in isolation inherit the
same bounds. We introduce \UniEdu{}, an
architecture-aware federated learning framework in
which each institution fine-tunes its own SLM with
a private LoRA adapter and exposes the model only
through a coarse \emph{architecture graph} whose
nodes are transformer blocks and whose features
summarize layer typology and the running statistics
of the LoRA-adjusted effective weights. A shared
graph attention network (GAT) encodes these
heterogeneous graphs into per-block embeddings, and
a client-local FiLM adapter translates each
embedding into a residual scale-and-shift
modulation of the corresponding block's hidden
states. Only the GAT parameters are federated, via
sample-size-weighted averaging, so SLMs of
fundamentally different architectures coexist in
one federation without dimension-matching
constraints. On a public testbed of MIT~18.657,
Stanford~CS229, and an arXiv ML paper corpus,
federation lifts every reference-based quality
metric over per-institution local training on a
held-out, cross-institution test set, with the
largest gains concentrated on the data-poorest
institution.
\end{abstract}
...
\textbf{The two questions we study} are an
\emph{equity} claim (does federation measurably
close the AQAG quality gap between resource-rich
and resource-constrained institutions, without
sharing curricular data?) and an
\emph{LLM-approximation} claim (does a federation
of heterogeneous SLMs match the QA-generation
quality of a single LLM on the same contexts?).
Both are stated formally in
Section~\ref{sec:scenario} and tested separately in
...
lectures, Pegasus-X for summarization-flavoured
...
institutions; clustered and personalized
...
\UniEdu{} is the educational-AQAG specialization of
...
Where \texttt{uGNN}/\texttt{UnifiedFL} re-parameterize
...
(Section~\ref{sec:public_testbed}); and a real-world
federation of five collaborating institutions
across five countries on three continents
(Section~\ref{sec:private_testbed}). Standard
...
\begin{enumerate}[leftmargin=2em, itemsep=2pt]
  ...
  federation of five institutions spanning Tunisia,
  Germany, Nigeria, Colombia, and Peru, comparing
...
\textbf{Small language models and LLM democratization.}
...
Most LLM-democratization work has focused on
distillation~\citep{hinton2015distilling} or quantized
inference, both of which produce one model trained on
one (typically large) corpus and one (typically
centralized) compute budget. Mixtures of
...
FL has been applied to language modeling and text
...
FL~\citep{sattler2020clustered} and personalized
...
channel from these works but specializes it to the
...
re-parameterized. Second, the shared GAT serves as a
...
tools have been recognized in policy
...
federation (Section~\ref{sec:equity}), so the equity
...
\begin{enumerate}[leftmargin=1.5em, itemsep=2pt]
  ...
  federated mixture of SLMs, taken collectively,
...
\subsection{Unified Federated Optimization}
\label{sec:fed_opt}
...
\red{TODO: confirm any client-sampling deviation from the all-clients-each-round default; verify final round counts and any warm-up.}

The full per-round pseudocode is given in
Algorithm~\ref{alg:unifiededu}
...
(Section~\ref{sec:public_testbed}) fixes a publicly
...
\emph{private testbed} (Section~\ref{sec:private_testbed})
runs the same framework across five real
collaborating institutions in Tunisia, Germany,
Nigeria, Colombia, and Peru training on their own
...
(Section~\ref{sec:models}), training procedure
(Section~\ref{sec:training}), baselines
(Section~\ref{sec:baselines}), and evaluation suite
(Section~\ref{sec:evaluation}). We report results on val
splits (Section~\ref{sec:results}); test splits are held
...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
  ...
  statistical learning theory and optimization. 110
...
pairs with GPT-4o, each labeled with its
...
\textbf{five real collaborating institutions}
spanning five countries on three continents
(Tunisia, Germany, Nigeria, Colombia, and Peru),
each contributing its own private machine-learning
curriculum. Concretely, each collaborator
(i)~runs the Phase~1 data-preparation pipeline locally
on their own materials (PDF lectures, PPTX decks,
textbook chapters, course assignments, research
papers), producing a JSON file conforming to the
same schema as the public testbed; (ii)~validates
their file with our schema validator;
(iii)~generates deterministic 3-fold CV splits with
the shared seed $=42$; and (iv)~ships only the
resulting checksum file to the coordinator for
cross-collaborator reproducibility verification.
\emph{No raw curricular data, no QA pairs, and no
model weights ever cross institutional boundaries.}

The institutions are deliberately heterogeneous on
three axes that are central to the equity
hypothesis (Table~\ref{tab:private_collaborators}).
First, GPU access spans the practical resource
range that constrains real institutions: one
collaborator runs on a high-end data-center GPU
(NVIDIA A100), one on a mid-range desktop GPU
(NVIDIA RTX~3090/4090), two on entry-level GPUs
(NVIDIA T4 / RTX~2080~Ti) via a Google Colab free
tier and personal hardware, and one whose GPU
profile is still pending submission of the
recruitment form (\red{TBD}, $\mathcal{I}_4$). Second, declared
training budgets range from 10--50 to 50--200 GPU
hours, forcing the federation to accommodate
short-budget participants without dragging
long-budget participants into wall-clock parity.
Third, dataset profiles vary across size (50--200
pages for three institutions, 200--500 for one,
\red{TBD} for $\mathcal{I}_4$), language coverage
(one English-only collaborator, three bilingual,
$\mathcal{I}_4$ \red{TBD}), academic level (mix of
mixed-level and undergraduate-advanced
contributions), and privacy posture (one
collaborator contributes fully public materials,
one a mix of public and private with
authorization, two contribute private institutional
materials with full authorization, $\mathcal{I}_4$
\red{TBD}). Each
institution is assigned a backbone family based on
its declared GPU tier and software preferences
(Table~\ref{tab:client_models}). The federation
involves no central training corpus, no shared
evaluation pool of raw text, and no exchange of
participant identities; the per-institution
profiles in Table~\ref{tab:private_collaborators}
are the only collaborator-level information that
leaves any single institution.

\begin{table}[t]
\centering
\footnotesize
\setlength{\tabcolsep}{3pt}
\caption{Private-testbed collaborators on the
heterogeneity axes most relevant to the equity
hypothesis. Identifiers are pseudonymized as
$\mathcal{I}_0$--$\mathcal{I}_4$. ``Privacy''
denotes the privacy posture of the contributed
curriculum; in every case the collaborator
explicitly authorized our research use.
\red{TBD} entries are pending the recruitment-form
return for $\mathcal{I}_4$.}
\label{tab:private_collaborators}
\begin{tabular}{@{}lllll@{}}
\toprule
\textbf{Inst.} & \textbf{Country} & \textbf{GPU tier}
& \textbf{Pages} & \textbf{Privacy} \\
\midrule
$\mathcal{I}_0$ & Tunisia  & mid-range   & 200--500 & private (auth.) \\
$\mathcal{I}_1$ & Germany  & high-end    & 50--200  & private (auth.) \\
$\mathcal{I}_2$ & Nigeria  & entry-level & 50--200  & mixed \\
$\mathcal{I}_3$ & Colombia & entry-level & 50--200  & public \\
$\mathcal{I}_4$ & Peru     & \red{TBD}   & \red{TBD} & \red{TBD} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Local optimization.} AdamW with peak learning
...
regimes (Section~\ref{sec:conditioning}): \texttt{baseline}
...
the comparisons in Section~\ref{sec:results} use
\texttt{topic} unless otherwise stated.

\textbf{Federated optimization.} \UniEdu{} runs $T=33$
federation rounds with $3$ local epochs per round and
sample-size-weighted FedAvg of the GAT parameters
$\boldsymbol{\phi}$ (Section~\ref{sec:fed_opt}). The total
...
The two hypotheses in Section~\ref{sec:scenario} require
...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
  ...
  parameters are exchanged. \red{TODO: implement FedKD in \texttt{unifiedfl/}.}
  ...
  public-testbed chunks (Section~\ref{sec:public_testbed})
  as the shared dataset. \red{TODO: implement FedMD in \texttt{unifiedfl/}.}
  \item \textbf{FedDF}~\citep{lin2020feddf}.
  Server-side ensemble distillation on an unlabeled
  ...
  than FedMD/FedDF's logit channel. \red{TODO: implement FedET in \texttt{unifiedfl/}.}
...
We compare the federated mixture of all SLMs (i.e.,
\UniEdu{} aggregated across institutions, not each
institution in isolation) against single-model LLMs
prompted zero-shot to produce a
$(q, a)$ pair from each test context, run through
the same evaluation suite:
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
  \item \textbf{GPT-4o} (headline ceiling).
...
(Section~\ref{sec:results_llm_approx}); these LLMs
do not appear as rows in the per-institution H1
tables.
...
\subsection{Question, Answer, and Joint Quality}
\label{sec:qa_quality}

\textbf{Question quality} is measured by
\textbf{RQUGE}~\citep{mohammadshahi2023rquge}, a
reference-free answerability score that runs a QA
model on $(q, c)$ and compares the predicted span to
the generated answer; we use a BERTScore-based span
scorer scaled to $[1,5]$. \textbf{Answer quality} is
measured by RAGAS
\textbf{Faithfulness}~\citep{es2023ragas} (NLI
entailment rate of atomic answer claims against the
context, via DeBERTa~\citep{he2021deberta}) and an
approximation of
\textbf{QAFactEval}~\citep{fabbri2022qafacteval}
based on UnifiedQA~\citep{khashabi2022unifiedqa}.
\textbf{Joint QA quality} is measured by Round-Trip
Consistency, inspired by
\citet{alberti2019synthetic} (token-F1 between the
generated answer and a re-derived answer obtained
by running a QA model on $(q, c)$), and by RAGAS
\textbf{Answer Relevancy}~\citep{es2023ragas}
(cosine similarity between the original question and
embeddings of $n$ reverse-generated questions). Full
formulations are deferred to
Appendix~\ref{app:eval_formulations}.
...
independent classifiers: a publicly available
\textbf{Bloom's BERT classifier}
(\texttt{cip29/bert-blooms-taxonomy-classifier})
fine-tuned on Bloom's-labeled question data, and a
\textbf{Bloom's LLM judge} (GPT-4o-mini queried with
a structured prompt that supplies verb-anchored
definitions of the six levels and returns a JSON
$\{\texttt{level}, \texttt{reason}\}$ object). The
Bloom's level is normalized to a continuous
\textbf{Educational Value Score} in $[0, 1]$ via
$\mathrm{EVS}(q) = (\hat{b}(q) - 1) / 5$.
...
The overall score is the four-dimension mean
normalised to $[0, 1]$ via $(\bar{s} - 1)/4$.
...
aggregate QA quality under paradigm $m$ (default:
LLM-judge overall); $\mathcal{C}_k$ its curricular
richness; $\mathcal{R}_k$ its compute resource
(trainable parameter count of the deployed backbone,
normalised by the largest in the federation). We
assess equity through three complementary
quantities. The \textbf{Pairwise Equity Score (PES)}
$E_{kl}^{(m)}$ rewards configurations that lift both
institutions of a $(k, l)$ pair simultaneously
relative to their gap in $(\mathcal{C}, \mathcal{R})$;
the central hypothesis is $E_{kl}^{(\uni)} >
E_{kl}^{(\fed)} > E_{kl}^{(\indiv)}$. The
\textbf{Anchor Question Equity Score} fixes a
shared anchor question $q^*$ and uses an LLM Equity
Judge to compare $s_l^{(\uni)}$, $s_k^{(\indiv)}$,
$s_l^{(\indiv)}$ for pairs $(k, l)$ with
$\mathcal{C}_k > \mathcal{C}_l$ and $\mathcal{R}_k =
\mathcal{R}_l$, predicting $s_l^{(\uni)} \approx
s_k^{(\indiv)} > s_l^{(\indiv)}$. The
\textbf{Knowledge Transfer Index} $\mathrm{KTI}(l
\leftarrow k) = Q_l^{(\uni)}(\mathcal{T}_{k\setminus
l}) - Q_l^{(\indiv)}(\mathcal{T}_{k\setminus l})$
isolates cross-client transfer on
topics exclusive to $k$'s curriculum; the
\UniEdu{}-noFiLM ablation
(Section~\ref{sec:baselines}) is the natural
control, since killing the conditioning channel
should drive KTI to zero. Full definitions,
derivations, and reference implementations are in
Appendix~\ref{app:equity_derivations}.

\red{TODO: confirm at least one collaborator pair satisfies $\mathcal{C}_k > \mathcal{C}_l, \mathcal{R}_k = \mathcal{R}_l$, or operationalize an approximate matching by binning $\mathcal{R}_k$.}
...
from val-set evaluation under each paradigm. We
apply a one-sided \textbf{paired Wilcoxon
signed-rank test} (preferred over the paired
$t$-test, since NLP quality scores are typically
non-Gaussian and bounded) with $H_1: \mu_{Q^{(\uni)}}
> \mu_{Q^{(\indiv)}}$. We report $p$-values at
$\alpha{=}0.05$ with \textbf{Bonferroni correction}
across the (metric, institution) grid,
\textbf{Cohen's} $d$ for effect size, and
\textbf{95\,\% bootstrap CIs} ($B{=}10{,}000$).

%% ════════════════════════════════════════════════════════════
\section{Results}
\label{sec:results}

The two hypotheses (Section~\ref{sec:scenario}) are
reported separately. Section~\ref{sec:results_equity}
tests \textbf{H1} (equity) by comparing \UniEdu{}
against per-institution local training. The result for the equity story is
\emph{per-institution}; GPT-4o does not appear as a
row. Section~\ref{sec:results_llm_approx} tests
...
(Section~\ref{app:final_ckpt}) holds the FINAL-checkpoint
...
LoRA + FiLM, Section~\ref{sec:method}) against the
...
the smallest-data and most under-represented
institution; federation improves all four
lightweight metrics simultaneously there by margins of
$+0.0166$ / $+0.0201$ / $+0.0058$ / $+0.0304$. On
each client's own local test
(Table~\ref{tab:val_test}, lower half), the
individual baseline retains its specialization
...
local data is scarcest. Federating BART, Flan-T5,
and LED simultaneously, a setting that prior
heterogeneous-FL work~\citep{wu2022fedkd,
li2019fedmd, lin2020feddf, cheng2022fedet} did not
attempt, therefore does not impose an accuracy cost
for architectural diversity: the data-poor client
is the one that benefits most from the shared
inductive bias. Supplementary breakdowns --- the in-distribution
local val/test view (Table~\ref{tab:val_test}),
pedagogical and LLM-judge metrics on the public
testbed (Tables~\ref{tab:public_pedagogical}--\ref{tab:public_llm_judge_dims}),
conditioning ablations (Table~\ref{tab:public_ablation}),
per-institution uplift (Table~\ref{tab:uplift}), and
the Knowledge Transfer Index
(Table~\ref{tab:kti}) --- are reported in
Appendix~\ref{app:supp_results}.

\red{TODO: re-run all (client, fold) cells with --no-heavy off + an OpenAI key to populate the LLM-based block (RTC, Faith., QAFE, RQUGE, Ans-Rel, Bloom-LLM EVS, LLM-judge overall). The current --fast-eval pass produced only the four lightweight columns.}
\begin{table*}[t]
\centering
\footnotesize
...
the right (Section~\ref{sec:evaluation}). Bold = better of
$\{\indiv, \uni\}$ per (institution, metric).
\textbf{Column abbreviations.} R-L: ROUGE-L F1.
B-4: BLEU-4. BERT: BERTScore-F1.
B\textsubscript{cls}: educational-value score
(EVS) from a local fine-tuned BERT Bloom's
classifier. B\textsubscript{LLM}: EVS from a
GPT-4o-mini Bloom's judge. AnsR: RAGAS Answer
Relevancy (cosine similarity between the original
question and a reverse-generated counterpart).
Faith: RAGAS Faithfulness (fraction of answer
claims entailed by the context). QAFE: QAFactEval
(approximate). RQU: RQUGE (in $[1,5]$, others in
$[0,1]$). RTC: Round-Trip Consistency. LLM-J:
GPT-4o LLM-judge overall, mean of four 1--5
rubric dimensions normalized to $[0,1]$.}
...
\red{TODO: private testbed (5 institutions) not yet collected; cells stay as "--" until Phase~2 of the private collaboration finishes. Methods rows (FedKD/MD/DF/ET) are also TODO; those baselines have not been run yet on the public testbed either.}
\begin{table*}[t]
\centering
\small
\caption{Equity comparison on the \textbf{private
testbed} (Section~\ref{sec:private_testbed}). Five
institutions (Section~\ref{sec:private_testbed},
Table~\ref{tab:private_collaborators}), six methods
per institution: the per-institution local baseline
\textbf{Indiv} (Section~\ref{sec:baselines}), four
heterogeneous-FL distillation baselines
\textbf{FedKD}~\citep{wu2022fedkd},
\textbf{FedMD}~\citep{li2019fedmd},
\textbf{FedDF}~\citep{lin2020feddf}, and
\textbf{FedET}~\citep{cheng2022fedet}, and our
\UniEdu{}. Column conventions follow
...
\red{TODO: per spec, no p-values until n>>3 folds; this table is left as a stub for the camera-ready when the full federation × seed grid (>=10 seeds per institution) has been run.}
\begin{table}[t]
\centering
\small
...
$\bar{E}$. The reference panel is the six
zero-shot LLMs \texttt{gpt-4o},
\texttt{gpt-4o-mini},
\texttt{Llama-3.1-8B-Instruct},
\texttt{Qwen-2.5-7B-Instruct},
\texttt{Mistral-7B-Instruct-v0.3}, and
\texttt{Gemini-1.5-Flash}. Each LLM is prompted
once per test context with the same fixed template
(greedy decoding,
\texttt{temperature{=}0.0},
\texttt{max\_tokens{=}256}), and the resulting
$(q, a)$ is scored with the same evaluation suite
as \UniEdu{}. The full LLM-judge prompt is
reproduced in Appendix~\ref{app:llm_judge_prompt}.

\red{TODO: UniEdu (fed.\ mixture) and GPT-4o rows populated for the 4 lightweight columns (R-L, B-4, BERT, B-EVS) on the public-testbed global mixed test set (916 rows, mean over 3 folds, mean over 3 backbone architectures for UniEdu). Heavy reference-free columns (RTC, Faith, QAFE, RQU) and the LLM-judge column (LLM-J) still pending. Other zero-shot LLM rows (GPT-4o-mini, Llama-3.1-8B-Inst, Qwen-2.5-7B-Inst, Mistral-7B-Inst, Gemini-1.5-Flash) not yet collected.}
\begin{table*}[t]
\centering
\footnotesize
\setlength{\tabcolsep}{3pt}
...
single-LLM baselines on the public-testbed
global mixed test set (916 rows, union of the three
per-institution test splits; mean over 3 folds). The
federated mixture row is \UniEdu{} averaged across
the three backbone architectures
(BART/Flan-T5/LED-base) deployed in the public
testbed. Column abbreviations match
Table~\ref{tab:headline_global}; B-EVS is the
local Bloom's classifier under EVS. Bold = best per metric.}
\label{tab:llm_approx}
\begin{tabular}{lccccccccc}
\toprule
\textbf{Method} &
\textbf{R-L} & \textbf{B-4} & \textbf{BERT} &
\textbf{RTC} & \textbf{Faith} & \textbf{QAFE} &
\textbf{RQU} & \textbf{B-EVS} & \textbf{LLM-J} \\
\midrule
\UniEdu{} (fed.\ mixture) & 0.4394 & 0.1688 & 0.8629 & -- & -- & -- & -- & 0.2296 & -- \\
\midrule
GPT-4o              & \textbf{0.4487} & \textbf{0.1869} & \textbf{0.8725} & -- & -- & -- & -- & \textbf{0.2624} & -- \\
GPT-4o-mini         & -- & -- & -- & -- & -- & -- & -- & -- & -- \\
Llama-3.1-8B-Inst   & -- & -- & -- & -- & -- & -- & -- & -- & -- \\
Qwen-2.5-7B-Inst    & -- & -- & -- & -- & -- & -- & -- & -- & -- \\
Mistral-7B-Inst     & -- & -- & -- & -- & -- & -- & -- & -- & -- \\
...
\red{TODO: one $(c, q^{\indiv}, q^{\uni}, q^{\text{GPT-4o}})$ sample per institution, chosen to illustrate the federation's effect. Place the full set in Appendix~\ref{app:qualitative}; reproduce one or two illustrative examples in the body.}
...
The KTI metric (Section~\ref{sec:equity};
Appendix~\ref{app:equity_derivations}) and the
...
educational AQAG setting. Taken together, these design choices position
\UniEdu{} as a practical foundation for equitable,
privacy-preserving AQAG across heterogeneous
...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
  ...
  \red{TODO: confirm whether to claim decoder-only support or scope strictly to encoder-decoder.}
  ...
  \red{[TODO: add appendix reference for per-(institution, fold) API costs]}.
\end{itemize}

%% ════════════════════════════════════════════════════════════
\section*{Acknowledgments}

\red{TODO: institutional / compute / funding acknowledgments; thanks to the public-testbed source materials (MIT 18.657, Stanford CS229, arXiv) and the five collaborating institutions of the private testbed for their participation; etc.}
...
(Section~\ref{sec:llm_judge}) is reproduced below verbatim. At
...
\red{TODO: two caveats from the launch-time verification audit (2026-05-08) that the table presents as if uniform across runs:   (a) client\_2/fold\_1/best/metrics\_val.json has the full       comprehensive eval suite (947 KB, includes faithfulness,       qafacteval, rquge, answer\_relevancy, llm\_judge\_*) — that       cell was either launched without --no-heavy or re-evaluated       comprehensively post hoc. The lightweight numbers used in       Tables 1/2 remain valid and apples-to-apples with the other       cells; only the "every cell ran with --fast-eval" claim is       technically false for that file.   (b) The deterministic-CUDA reproducibility setup (commit e079ace,       2026-05-06 16:11 BST) was active for all DoC-launched runs       (fed fold2/3, indiv c2/f3); for the earlier Colab-launched       runs the launch commit is not on-machine-recoverable, so       deterministic mode is unconfirmed for those. Practical       output deltas are negligible but the literal "fully       deterministic, seed=42" framing applies cleanly only to the       DoC-launched cells.}

Table~\ref{tab:hyperparams_full} reports every
hyperparameter used across the public-testbed runs.
Numeric values are taken verbatim from the slurm
launch logs (DoC-launched runs) or the script
defaults at the launch commit; LoRA settings were
spot-checked against the saved
\texttt{adapter\_config.json} files. Per-client
backbone specifications are in
Table~\ref{tab:client_models}; everything else
below is identical across the three institutions.
...
\caption{Public-testbed hyperparameters. Identical
across the three clients unless noted. Per-client
backbone, LoRA-target naming, and $d_{\text{model}}$
appear in Table~\ref{tab:client_models}.}
\label{tab:hyperparams_full}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.18\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.48\textwidth}@{}}
\toprule
\textbf{Block} & \textbf{Hyperparameter} & \textbf{Value} \\
\midrule

\multirow{8}{*}{\textbf{Shared}}
  & Random seed                                & $42$ \\
  & Batch size                                 & $4$ \\
  & Max input length                           & $512$ tokens \\
  & Max target length                          & $128$ tokens \\
  & Optimizer                                  & AdamW (weight decay $0.01$) \\
  & LR schedule                                & cosine with 10\% linear warm-up \\
  & Warm-up ratio                              & $0.1$ \\
  & Gradient clipping (L2 norm)                & $1.0$ \\
\midrule

\multirow{4}{*}{\textbf{LoRA (all clients)}}
  & Rank $r$                                   & $16$ \\
  & Scaling $\alpha$                           & $32$ \\
  & Dropout                                    & $0.1$ \\
  & Bias                                       & none \\
...
\multirow{6}{*}{\textbf{Individual training}}
  & Max epochs                                 & $100$ \\
  & Learning rate (LoRA)                       & $3 \times 10^{-4}$ \\
  & Early-stop patience                        & $10$ epochs \\
  & Early-stop $\Delta_{\min}$                 & $10^{-4}$ \\
  & Early-stop metric                          & greedy ROUGE-L on $50$ fixed val samples \\
  & Periodic checkpoint                        & every $5$ epochs \\
\midrule

\multirow{6}{*}{\textbf{Eval (Indiv, baseline)}}
  & Decoding                                   & diverse beam search \\
  & Beams                                      & $10$ \\
  & Beam groups                                & $5$ \\
  & Diversity penalty                          & $1.0$ \\
  & Returned sequences / context               & $5$ (Hungarian-matched to refs) \\
  & Eval suite                                 & lightweight (\texttt{--fast-eval}) \\
\midrule

\multirow{8}{*}{\textbf{Federated (\UniEdu{})}}
  & Rounds $T$                                 & $33$ \\
  & Local epochs / round                       & $3$ \\
  & Learning rate (LoRA)                       & $3 \times 10^{-4}$ \\
  & Learning rate (GAT)                        & $1 \times 10^{-3}$ \\
  & Learning rate (FiLM)                       & $1 \times 10^{-3}$ \\
  & Best-snapshot metric                       & avg greedy ROUGE-L on $50$ val samples / client, post-aggregation \\
  & Periodic checkpoint                        & every $5$ rounds \\
  & FedAvg scope                               & sample-size-weighted; GAT only (LoRA + FiLM client-local) \\
\midrule

\multirow{6}{*}{\textbf{GAT (\UniEdu{})}}
  & Layers                                     & $3$ \\
  & Hidden                                     & $64$ \\
  & Heads (layers 1--2, concat)                & $4$ \\
  & Heads (layer 3, no concat)                 & $1$ \\
  & Input feature dimension                    & $16$ \\
  & Activation / dropout                       & ELU / $0.1$ \\
\midrule

\multirow{3}{*}{\textbf{FiLM (\UniEdu{})}}
  & Hidden                                     & $128$ \\
  & Output dimension                           & $2 \, d_{\text{model}}^{(k)}$ per client \\
  & $\alpha_{\text{init}}$                     & $0$ (identity at init) \\
\midrule

\multirow{4}{*}{\textbf{Mixed precision}}
  & Base-model dtype                           & FP32 (frozen) \\
  & LoRA adapter dtype                         & FP32 \\
  & Autocast                                   & BF16 on CUDA when supported, else FP32 \\
  & GradScaler                                 & disabled (BF16 has full FP32 range) \\
\midrule

\multirow{6}{*}{\textbf{Reproducibility}}
  & \texttt{torch} / \texttt{numpy} / \texttt{random} seeds & $42$ \\
  & cuDNN deterministic                        & \texttt{True} \\
  & cuDNN benchmark                            & \texttt{False} \\
  & \texttt{use\_deterministic\_algorithms}    & \texttt{True} (\texttt{warn\_only=True}) \\
  & \texttt{CUBLAS\_WORKSPACE\_CONFIG}         & \texttt{:4096:8} \\
  & \texttt{PYTHONHASHSEED}                    & \texttt{42} \\

\bottomrule
\end{tabular}
\end{table*}

For the dataset-statistics side, the per-fold sample
counts on the public testbed are:
$\mathcal{I}_0$ (MIT, BART), fold~1 train/val
$313 / 154$, fold~2 $310 / 157$, fold~3 $311 / 156$;
$\mathcal{I}_1$ (Stanford, Flan-T5), fold~1
$320 / 158$, fold~2 $321 / 157$, fold~3 $315 / 163$;
$\mathcal{I}_2$ (ML papers, LED), fold~1
$2854 / 1428$, fold~2 $2857 / 1425$, fold~3
$2853 / 1429$. Per-client local test sets are
$|\mathcal{T}_0|{=}80$, $|\mathcal{T}_1|{=}81$,
$|\mathcal{T}_2|{=}755$; the global test pool
$\mathcal{T}^{\text{global}}$ is their concatenation
($916$ samples).

\section{Federated Algorithm Pseudocode}
\label{app:algorithm}

Algorithm~\ref{alg:unifiededu} formalises one round
of the unified federated optimization described in
Section~\ref{sec:fed_opt}: each institution refreshes
its architecture-graph node features at every
mini-batch, runs the shared GAT to obtain per-block
embeddings, updates its private LoRA and FiLM
parameters jointly with the local copy of the GAT
parameters, and uploads only $\boldsymbol{\phi}$ to
the server for sample-size-weighted FedAvg
aggregation.
...
\section{Evaluation Metric Formulations}
\label{app:eval_formulations}

This appendix gives the formal definitions of every
non-equity evaluation metric whose body description
(Section~\ref{sec:evaluation}) is prose only.
Equity-metric derivations are in
Appendix~\ref{app:equity_derivations}.

\paragraph{RQUGE (Question Quality).}
...
\paragraph{RAGAS Faithfulness (Answer Quality).}
...
\paragraph{Round-Trip Consistency (Joint QA Quality).}
RTC, inspired by \citet{alberti2019synthetic}:
\begin{equation}
  \mathrm{RTC}(q, a, c)
...
\paragraph{RAGAS Answer Relevancy (Joint QA Quality).}
...
\paragraph{Bloom's BERT Classifier (Pedagogical Quality).}
A BERT-base classifier
(\texttt{cip29/bert-blooms-taxonomy-classifier})
fine-tuned on Bloom's-labeled question data
predicts a Bloom level $\hat{b}(q) \in \{1,
\ldots, 6\}$:
\begin{equation}
  \hat{b}(q) = \argmax_{b}\,
  ...
  \label{eq:bloom_bert}
\end{equation}
The predicted level is normalised to a continuous
\textbf{Educational Value Score} in $[0, 1]$:
\begin{equation}
...
\paragraph{Paired Wilcoxon Signed-Rank Test
(Statistical Validation).}
The body Section~\ref{sec:stats} uses a one-sided
paired Wilcoxon signed-rank test with the
hypotheses
\begin{equation}
...
\red{TODO: insert the per-source chunk-count and QA-pair-count table; insert the checksum file in verbatim form.}
...
\red{TODO: insert side-by-side comparison table once Phase~2 finishes for both variants. Expectation: the GAT-mediated transfer should remain measurable in the balanced setting, isolating the federation effect from raw data-quantity effects.}

\section{Architecture Graph Examples}
\label{app:graph_examples}

For each SLM family, we visualize the architecture
graph our constructor builds (nodes colored by
...
vectors are 16-dimensional (Section~\ref{sec:arch_graph}).

\red{TODO: insert one graph figure per family (BART, Flan-T5, LED, Pegasus-X, ProphetNet) generated via \texttt{unifiedfl/visualize\_graph.py}.}
...
\red{TODO: report final wall-clock and API totals per testbed; include a footnote on how the cost compares against running GPT-4o as the QA-generator itself (the natural opponent in our headline hypothesis).}
...
\red{TODO: hand-pick examples post-results; format each as (context, indiv-QA, uni-QA, GPT-4o-QA) with a short comment.}
...
the three equity metrics in Section~\ref{sec:equity}: the
...
(a per-institution proxy combining chunk count and
topic diversity; \red{TODO: spell out the exact
formula}), normalized to $[0, 1]$ across the
federation. $\mathcal{R}_k$ is the
trainable-parameter count of $k$'s SLM, normalized
...
\end{equation}
that is, the federated low-resource client matches
the high-resource individual, and beats the
...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
  ...
  \red{[TODO: add table of anchor questions]}.
\end{itemize}
...
FedMD, FedDF, and FedET require an unlabeled
public dataset for their distillation step. We use
the public-testbed chunks
(Section~\ref{sec:public_testbed}) as that dataset: each
...
\red{TODO: same OpenAI-enabled re-run as the main paper would also fill the LLM-based block here.}
\begin{table*}[t]
\centering
\footnotesize
\setlength{\tabcolsep}{2pt}
...
$\{\indiv, \uni\}$ per (institution, metric).
Column abbreviations are defined in the caption of
Table~\ref{tab:headline_global}.}
...
folds. Bold = better of $\{\indiv, \uni\}$. Column
abbreviations are defined in the caption of
Table~\ref{tab:headline_global}.}
...
\label{app:supp_results}

This appendix collects the supplementary result
tables referenced from
Section~\ref{sec:results_equity}: in-distribution
local val/test breakdown
(Table~\ref{tab:val_test}), pedagogical and
LLM-judge metrics on the public testbed
(Tables~\ref{tab:public_pedagogical}--\ref{tab:public_llm_judge_dims}),
the \UniEdu{}-static and \UniEdu{}-noFiLM
conditioning ablations
(Table~\ref{tab:public_ablation}), per-institution
uplift on the private testbed
(Table~\ref{tab:uplift}), and the
direction-resolved Knowledge Transfer Index
(Table~\ref{tab:kti}).

\red{TODO: same back-fill applies — local val and local test cells currently populated for lightweight metrics only.}
\begin{table*}[t]
\centering
\footnotesize
\setlength{\tabcolsep}{2pt}
...
metric). Column abbreviations are defined in the
caption of Table~\ref{tab:headline_global}.}
\label{tab:val_test}
\begin{tabular}{ll cccc ccccccc}
\toprule
\multirow{2}{*}{\textbf{Inst.}} &
...
\red{TODO: Bloom-BERT EVS values for \UniEdu{} can be filled from the blooms\_cls\_evs\_mean key already present in metrics\_*.json (best ckpt, global test). Bloom-LLM EVS and LLM-judge columns require a re-run with the OpenAI key enabled.}
\begin{table*}[t]
\centering
\small
...
Method rows: \textbf{Indiv} (Section~\ref{sec:baselines}),
\textbf{FedKD}~\citep{wu2022fedkd},
\textbf{FedMD}~\citep{li2019fedmd},
\textbf{FedDF}~\citep{lin2020feddf},
\textbf{FedET}~\citep{cheng2022fedet}, and
\UniEdu{}.}
...
\red{TODO: four-dim LLM-judge breakdown not yet populated; needs the same OpenAI-enabled re-run flagged for tab:headline\_global.}
\begin{table}[t]
\centering
\footnotesize
...
\red{TODO: UniEdu-static (graph features frozen at round 0) and UniEdu-noFiLM (GAT trained but not fed into the LM) ablations not yet run; one-fold pilot per ablation will go here.}
\begin{table}[t]
\centering
\small
...
\red{TODO: private-testbed uplift; awaits private Phase~2 outputs.}
\begin{table}[t]
\centering
\footnotesize
\setlength{\tabcolsep}{4pt}
...
\red{TODO: KTI requires per-direction held-out eval on $T_{k\setminus l}$ chunks; eval pool not yet curated.}
\begin{table}[t]
\centering
\small
