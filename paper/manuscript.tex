...
\multirow{8}{*}{\textbf{Federated (\UniEdu{})}}
  ...
  & Best-snapshot metric                       & avg greedy ROUGE-L on $50$ fixed val samples / client (post-aggregation) \\
  & Periodic checkpoint                        & every $5$ rounds \\
  & FedAvg scope                               & sample-size-weighted; GAT params only (LoRA + FiLM stay client-local) \\
\midrule
...
\multirow{4}{*}{\textbf{Mixed precision}}
  ...
  & Autocast                                   & BF16 on CUDA when supported; FP32 otherwise \\
  & GradScaler                                 & disabled (BF16 has FP32-range exponent) \\
\midrule

\multirow{5}{*}{\textbf{Reproducibility}}
  & \texttt{torch.manual\_seed} / \texttt{numpy.random.seed} / \texttt{random.seed} & $42$ \\
  & cuDNN deterministic                        & \texttt{True} \\
  & cuDNN benchmark                            & \texttt{False} \\
  & \texttt{torch.use\_deterministic\_algorithms} & \texttt{True} (\texttt{warn\_only=True}) \\
  & \texttt{CUBLAS\_WORKSPACE\_CONFIG}, \texttt{PYTHONHASHSEED} & \texttt{:4096:8}, \texttt{42} \\
