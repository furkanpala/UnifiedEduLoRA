...
\title{\UniEdu{}: Equitable Automated Q\&A Generation\\
       via Architecture-Aware Federated Learning}
...
\begin{algorithm}[t]
\caption{\UniEdu{}: architecture-aware federated AQAG.}
...
We compare \UniEdu{} against:
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
  \item \textbf{Individual SLM (lower bound).} Each
  ...
  \item \textbf{LoRA-FedAvg (within-family).} For any
  pair of institutions sharing an SLM family,
  aggregate LoRA weights via vanilla
  FedAvg~\citep{mcmahan2017communication}. Anchors the
  comparison against the standard FL practice when
  architectures match. \red{TODO: confirm whether any
  pair in the private testbed shares an architecture;
  if not, report this on synthetic same-architecture
  splits drawn from the public testbed.}
  \item \textbf{\UniEdu{}-static}: architecture-graph
  features fixed at round 0, no per-batch refresh of
  effective-weight statistics. Ablates the dynamic
  feature update.
  \item \textbf{\UniEdu{}-noFiLM}: GAT trained but its
  embeddings unused at the LM forward. Ablates the
  conditioning channel.
  \item \textbf{\UniEdu{} (full)}: GAT-FedAvg, LoRA
  and FiLM client-local, per-batch effective-weight
  refresh.
  \item \textbf{Single-LLM ceiling.} Zero-shot GPT-4o
  prompted to produce a (question, answer) pair from
  each test context, run through the same evaluation
  suite. The single-LLM number is what we ask the SLM
  federation to approach.
\end{itemize}
We also report a \textbf{diverse-LLM panel} ---
zero-shot Llama-3.1-8B-Instruct, Qwen-2.5-7B-Instruct,
Mistral-7B-Instruct, and Gemini-1.5-Flash --- as
...
We report results across two testbeds and three
training paradigms. The single-LLM ceiling
(\textbf{GPT-4o}) is reported per institution by
prompting GPT-4o to produce a $(q, a)$ from each
test context and running the same evaluation suite.
\textbf{Indiv} is each institution's locally
fine-tuned SLM (no federation). \textbf{\UniEdu{}}
is the full framework. All metrics are reported as
mean over 3-fold cross-validation on the val set
unless otherwise stated. The text-overlap metrics
(ROUGE-L, BLEU-4, BERTScore-F1) are higher-is-better
in $[0, 1]$. RTC, Faithfulness, QAFactEval, and
Answer-Relevancy are higher-is-better in $[0, 1]$.
RQUGE is higher-is-better in $[1, 5]$. Bloom EVS is
higher-is-better in $[0, 1]$ (Eq.~\ref{eq:evs}).
LLM-judge overall is higher-is-better in $[0, 1]$
(Eq.~\ref{sec:llm_judge}).

\subsection{Public Testbed}
\label{sec:results_public}

\red{TODO: fill in once Phase~2 finishes for all
three public-testbed clients. Numbers below are
placeholders.}

\begin{table*}[t]
\centering
\small
\caption{Public testbed (\S\ref{sec:public_testbed}):
text-overlap and reference-free QA-quality metrics on
the val set. Each cell is the mean over 3 folds.
Bold = best per (institution, metric);
\underline{underline} = approaches GPT-4o ($\Delta < 0.02$).}
\label{tab:public_quality}
\begin{tabular}{llccccccc}
\toprule
\textbf{Institution} & \textbf{Paradigm} &
\textbf{ROUGE-L} & \textbf{BLEU-4} & \textbf{BERTScore} &
\textbf{RTC} & \textbf{Faith.} & \textbf{QAFactEval} &
\textbf{RQUGE} \\
\midrule
\multirow{3}{*}{$\mathcal{I}_0$ (BART-base, MIT)}
  & Indiv      &     --     &     --     &     --     &     --     &     --     &     --     &     --     \\
  & \UniEdu{}  &     --     &     --     &     --     &     --     &     --     &     --     &     --     \\
  & GPT-4o     &     --     &     --     &     --     &     --     &     --     &     --     &     --     \\
\midrule
\multirow{3}{*}{$\mathcal{I}_1$ (Flan-T5-base, Stanford)}
  & Indiv      &     --     &     --     &     --     &     --     &     --     &     --     &     --     \\
  & \UniEdu{}  &     --     &     --     &     --     &     --     &     --     &     --     &     --     \\
  & GPT-4o     &     --     &     --     &     --     &     --     &     --     &     --     &     --     \\
\midrule
\multirow{3}{*}{$\mathcal{I}_2$ (LED-base, ML papers)}
  & Indiv      &     --     &     --     &     --     &     --     &     --     &     --     &     --     \\
  & \UniEdu{}  &     --     &     --     &     --     &     --     &     --     &     --     &     --     \\
  & GPT-4o     &     --     &     --     &     --     &     --     &     --     &     --     &     --     \\
\midrule
\multirow{3}{*}{Mean across $\mathcal{I}_{0,1,2}$}
  & Indiv      &     --     &     --     &     --     &     --     &     --     &     --     &     --     \\
  & \UniEdu{}  &     --     &     --     &     --     &     --     &     --     &     --     &     --     \\
  & GPT-4o     &     --     &     --     &     --     &     --     &     --     &     --     &     --     \\
\bottomrule
...
\centering
\small
\caption{Public testbed: pedagogical and LLM-judge
metrics. Bloom-EVS is in $[0, 1]$; LLM-judge is the
overall (mean across the four rubric dimensions),
also in $[0, 1]$.}
...
\centering
\small
\caption{Public testbed: LLM-judge per-dimension
breakdown for the \UniEdu{} federation, on a strict
1--5 scale (higher is better).}
\label{tab:public_llm_judge_dims}
\begin{tabular}{lcccc}
\toprule
\textbf{Institution} & \textbf{Context grnd.} &
\textbf{Edu.\ value} & \textbf{Ans.\ correct.} &
\textbf{Ans.\ relev.} \\
\midrule
$\mathcal{I}_0$ & -- & -- & -- & -- \\
$\mathcal{I}_1$ & -- & -- & -- & -- \\
...
\centering
\small
\caption{Public testbed: ablation. \UniEdu{}-static
freezes the architecture-graph features at round~0;
\UniEdu{}-noFiLM trains the GAT but does not feed its
embeddings into the LM. Drop relative to full
\UniEdu{} on the LLM-judge overall.}
...
\subsection{Private Testbed}
\label{sec:results_private}

\red{TODO: fill in once Phase~2 finishes for all
five private-testbed collaborators.}

\begin{table*}[t]
\centering
\small
\caption{Private testbed (\S\ref{sec:private_testbed}):
five real institutions training on private curricula.
Same column layout as Table~\ref{tab:public_quality}.}
\label{tab:private_quality}
\begin{tabular}{llccccccc}
\toprule
\textbf{Institution / SLM} & \textbf{Paradigm} &
\textbf{ROUGE-L} & \textbf{BLEU-4} & \textbf{BERTScore} &
\textbf{RTC} & \textbf{Faith.} & \textbf{QAFactEval} &
\textbf{RQUGE} \\
\midrule
\multirow{3}{*}{$\mathcal{I}_0$ / BART-base}
  & Indiv      & -- & -- & -- & -- & -- & -- & -- \\
  & \UniEdu{}  & -- & -- & -- & -- & -- & -- & -- \\
  & GPT-4o     & -- & -- & -- & -- & -- & -- & -- \\
\midrule
\multirow{3}{*}{$\mathcal{I}_1$ / Flan-T5-base}
  & Indiv      & -- & -- & -- & -- & -- & -- & -- \\
  & \UniEdu{}  & -- & -- & -- & -- & -- & -- & -- \\
  & GPT-4o     & -- & -- & -- & -- & -- & -- & -- \\
\midrule
\multirow{3}{*}{$\mathcal{I}_2$ / LED-base-16384}
  & Indiv      & -- & -- & -- & -- & -- & -- & -- \\
  & \UniEdu{}  & -- & -- & -- & -- & -- & -- & -- \\
  & GPT-4o     & -- & -- & -- & -- & -- & -- & -- \\
\midrule
\multirow{3}{*}{$\mathcal{I}_3$ / Pegasus-X-base}
  & Indiv      & -- & -- & -- & -- & -- & -- & -- \\
  & \UniEdu{}  & -- & -- & -- & -- & -- & -- & -- \\
  & GPT-4o     & -- & -- & -- & -- & -- & -- & -- \\
\midrule
\multirow{3}{*}{$\mathcal{I}_4$ / ProphetNet-large}
  & Indiv      & -- & -- & -- & -- & -- & -- & -- \\
  & \UniEdu{}  & -- & -- & -- & -- & -- & -- & -- \\
  & GPT-4o     & -- & -- & -- & -- & -- & -- & -- \\
\midrule
\multirow{3}{*}{Mean across $\mathcal{I}_{0..4}$}
  & Indiv      & -- & -- & -- & -- & -- & -- & -- \\
  & \UniEdu{}  & -- & -- & -- & -- & -- & -- & -- \\
  & GPT-4o     & -- & -- & -- & -- & -- & -- & -- \\
\bottomrule
...
- Q^{(\indiv)}$ on selected metrics (private
testbed). A positive value indicates that the
institution's SLM benefits from the federation.}
\label{tab:uplift}
\begin{tabular}{lccc}
\toprule
\textbf{Institution} & $\Delta\,$\textbf{ROUGE-L} &
$\Delta\,$\textbf{LLM-judge} & $\Delta\,$\textbf{Bloom-EVS} \\
\midrule
$\mathcal{I}_0$ (BART)        & -- & -- & -- \\
...
$\mathcal{T}_{k\setminus l}$ (the held-out chunks
...
\begin{tabular}{llccc}
\toprule
\textbf{Direction $(l \leftarrow k)$} & \textbf{Paradigm} &
\textbf{$Q_l^{\indiv}$} & \textbf{$Q_l^{\uni}$} &
$\mathrm{KTI}$ \\
\midrule
...
\red{TODO: paired Wilcoxon signed-rank test
$(H_1: \mu_{\uni} > \mu_{\indiv})$ at
$\alpha=0.05$ with Bonferroni correction across the
(metric, institution) grid; Cohen's $d$ for effect
size; $95\%$ bootstrap CIs ($B=10{,}000$).}
...
LLM-judge overall, private testbed.}
\label{tab:wilcoxon}
\begin{tabular}{lccc}
\toprule
...
q^{\text{GPT-4o}})$ sample per institution, chosen to
illustrate the federation's effect. Place the full set
in Appendix~\ref{app:qualitative}; reproduce one or
two illustrative examples in the body.}
...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
  ...
  current experiments cover T5, BART, and LED.
  Decoder-only causal LMs are supported in principle
  (FiLM hooks would attach to causal blocks
  identically) but are not exercised in this paper.
  ...
  \item \textbf{Federation scale.} The empirical study
  involves three institutions. Larger federations
  ($m \geq 10$) would test whether GAT averaging
  scales gracefully under more diverse architecture
  distributions; we leave this to future work.
...
acknowledgments; thanks to the three collaborating
institutions for their participation; etc.}
...
\section{Equity-Metric Derivations}
\label{app:equity_derivations}

We provide derivations for the three equity metrics
in \S\ref{sec:equity}: the \emph{Pairwise Equity
Score} $E_{kl}^{(m)}$
(Eq.~\ref{eq:pairwise_equity}), the
\emph{Knowledge Transfer Index}
$\mathrm{KTI}(l\leftarrow k)$ (Eq.~\ref{eq:kti}),
and the \emph{Anchor Question Equity}
score~(Eq.~\ref{eq:anchor_hypothesis}). For each, we
state the assumptions, the operationalisation, and
the failure mode when the assumption is violated
(e.g., what KTI measures when
$\mathcal{T}_{k\setminus l}$ is empty, or how
$E_{kl}^{(m)}$ behaves when
$|\mathcal{C}_k - \mathcal{C}_l| \to 0$).

\red{TODO: write derivations once metric values are
populated.}
