...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
  ...
  parameters are exchanged. Re-implemented for our
  AQAG-on-SLM setting; details in
  Appendix~\ref{app:fedkd_implementation}.
  \red{TODO: implement FedKD in unifiedfl/.}
  \item \textbf{Benchmark B3.} \red{TODO: confirm with
  Furkan which heterogeneous-FL or personalised-FL
  baseline to include here (candidates: HeteroFL,
  InclusiveFL, FedRep, pFedHN). Insert citation and
  one-line description.}
  \item \textbf{Benchmark B4.} \red{TODO: TBD.}
  \item \textbf{Benchmark B5.} \red{TODO: TBD.}
...
training and other federation methods. The result
for the equity story is \emph{per-institution};
GPT-4o does not appear as a row.
\S\ref{sec:results_llm_approx} tests \textbf{H2}
(LLM approximation) by aggregating \UniEdu{}'s
output across the federation and comparing the
aggregate against single-LLM zero-shot baselines.
All metrics are reported as the mean over 3-fold
cross-validation on the val set unless otherwise
stated. Score directions: ROUGE-L / BLEU-4 /
BERTScore-F1 / RTC / Faithfulness / QAFactEval /
Answer-Relevancy / Bloom-EVS / LLM-judge overall are
all higher-is-better in $[0, 1]$. RQUGE is
...
We compare \UniEdu{} against \textbf{Indiv},
\textbf{FedKD}~\citep{wu2022fedkd}, and three
additional benchmarks (\S\ref{sec:baselines})
on the public testbed (Table~\ref{tab:equity_public})
and the private testbed
(Table~\ref{tab:equity_private}).

\begin{table*}[t]
\centering
\small
\caption{Equity comparison on the \textbf{public
testbed} (\S\ref{sec:public_testbed}). Per-institution
QA-quality metrics on the val set (mean over 3 folds).
$\bar{E}$ is the institution's mean Pairwise Equity
Score against the other clients in the same paradigm
(Eq.~\ref{eq:pairwise_equity}; higher = both clients
do well \emph{despite} resource asymmetry).
GPT-4o is intentionally not a row here --- the
LLM-approximation comparison is reported separately in
Table~\ref{tab:llm_approx}. Bold = best per
(institution, metric).}
\label{tab:equity_public}
...
\textbf{RQUGE} & $\bar{E}$ \\
\midrule
\multirow{6}{*}{$\mathcal{I}_0$ (BART, MIT)}
  & Indiv          & -- & -- & -- & -- & -- & -- & -- & -- \\
  & FedKD          & -- & -- & -- & -- & -- & -- & -- & -- \\
  & B3 \red{[TBD]} & -- & -- & -- & -- & -- & -- & -- & -- \\
  & B4 \red{[TBD]} & -- & -- & -- & -- & -- & -- & -- & -- \\
  ...
  & \UniEdu{}      & -- & -- & -- & -- & -- & -- & -- & -- \\
\midrule
\multirow{6}{*}{$\mathcal{I}_1$ (Flan-T5, Stanford)}
  & Indiv          & -- & -- & -- & -- & -- & -- & -- & -- \\
  & FedKD          & -- & -- & -- & -- & -- & -- & -- & -- \\
  & B3 \red{[TBD]} & -- & -- & -- & -- & -- & -- & -- & -- \\
  & B4 \red{[TBD]} & -- & -- & -- & -- & -- & -- & -- & -- \\
  ...
  & \UniEdu{}      & -- & -- & -- & -- & -- & -- & -- & -- \\
\midrule
\multirow{6}{*}{$\mathcal{I}_2$ (LED, ML papers)}
  & Indiv          & -- & -- & -- & -- & -- & -- & -- & -- \\
  & FedKD          & -- & -- & -- & -- & -- & -- & -- & -- \\
  & B3 \red{[TBD]} & -- & -- & -- & -- & -- & -- & -- & -- \\
  & B4 \red{[TBD]} & -- & -- & -- & -- & -- & -- & -- & -- \\
  ...
  & \UniEdu{}      & -- & -- & -- & -- & -- & -- & -- & -- \\
\midrule
\multirow{6}{*}{Mean across $\mathcal{I}_{0,1,2}$}
  & Indiv          & -- & -- & -- & -- & -- & -- & -- & -- \\
  & FedKD          & -- & -- & -- & -- & -- & -- & -- & -- \\
  & B3 \red{[TBD]} & -- & -- & -- & -- & -- & -- & -- & -- \\
  & B4 \red{[TBD]} & -- & -- & -- & -- & -- & -- & -- & -- \\
...
as Table~\ref{tab:equity_public} with five
...
Same set of methods as Table~\ref{tab:equity_public}.}
...
Tables~\ref{tab:equity_public}--\ref{tab:equity_private}
...
\section{FedKD Re-implementation Details}
\label{app:fedkd_implementation}

\red{TODO: describe how FedKD~\citep{wu2022fedkd}
was re-implemented for our SLM-AQAG setting:
(i)~mentor/mentee pair per client, both seeded from
the institution's local SLM checkpoint;
(ii)~local distillation loss (KL on next-token
distributions);
