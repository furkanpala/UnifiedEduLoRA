...
at scale, but the quality of any locally trained
AQAG system is bounded by the volume and diversity
of one institution's curriculum and by the compute
available to fine-tune a generative model. This
produces an \textbf{equity gap}: well-resourced
institutions train pedagogically rich generators,
while resource-constrained institutions are
disadvantaged by both narrow curricula and
lightweight models. Large language models (LLMs)
would close that gap in principle, but their
compute, licensing, and data-governance requirements
put direct deployment out of reach for most
institutions. Small language models (SLMs)
encoder-decoder backbones in the 100M--500M
parameter range run on commodity hardware and keep
curricular data local, but in isolation they
inherit the same bounds. We test two complementary
hypotheses: \textbf{(H1, equity)} a federation of
SLMs reduces the AQAG quality gap between
resource-rich and resource-constrained institutions
while keeping all curricular data private; and
\textbf{(H2, LLM approximation)} the resulting
federation approximates the QA-generation quality
of a single LLM on the same contexts. We introduce
\UniEdu{}, an architecture-aware federated learning
framework in which each institution fine-tunes its
own SLM with a private LoRA adapter and exposes the
model to the federation only through a coarse
\emph{architecture graph} whose nodes are
transformer blocks and whose features summarize
layer typology and the running statistics of the
LoRA-adjusted effective weights. A shared graph
attention network (GAT) encodes these heterogeneous
graphs into per-block embeddings, and a
client-local FiLM adapter translates each embedding
into a residual scale-and-shift modulation of the
corresponding block's hidden states. Only the GAT
parameters are federated, via sample-size-weighted
averaging, so SLMs of fundamentally different
architectures coexist in one federation without
dimension-matching constraints. On a public testbed
of MIT~18.657, Stanford~CS229, and an arXiv ML
paper corpus, federation lifts every reference-based
quality metric over per-institution local training
on a held-out, cross-institution test set, with the
largest gains concentrated on the data-poorest
institution. We further compare the federated
mixture against GPT-4o on the same contexts and
measure QA quality across reference-based,
reference-free, pedagogical, and LLM-judge axes,
together with three equity-specific metrics
(Section~\ref{sec:equity}).
\red{TODO: tighten the H2 quantitative summary once the OpenAI-enabled comprehensive eval pass and the diverse-LLM zero-shot panel are run.}
\end{abstract}
...
(Section~\ref{sec:results_llm_approx},
...
\red{TODO: insert Table comparing the diverse-LLM panel against \UniEdu{} on the public-testbed mean.}
