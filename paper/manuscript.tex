...
(\S\ref{sec:private_testbed}). Standard
...
LoRA + FiLM, \S\ref{sec:method}) against the
...
the right (\S\ref{sec:evaluation}). Bold = better of
...
testbed} (\S\ref{sec:private_testbed}). Five
institutions (\S\ref{sec:private_testbed},
Table~\ref{tab:private_collaborators}), six methods
per institution: the per-institution local baseline
\textbf{Indiv} (\S\ref{sec:baselines}), four
...
Method rows: \textbf{Indiv} (\S\ref{sec:baselines}),
\textbf{FedKD}~\citep{wu2022fedkd},
\textbf{FedMD}~\citep{li2019fedmd},
